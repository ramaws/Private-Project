		 ...
		 <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2 zeroPadding rightpadding">
			<div id="sidebar-wrapper" class="sidebar-toggle">
				<ul class="sidebar-nav">
					...
					<li class="btnActive"><a href='#'>Machinetool Type</a></li>
					<li><a href='<c:url value="/config/machine_tools"/>'>Machinetool-MDU</a>
					</li>
				</ul>
			</div>
			</div>	
			<div class="col-lg-10 col-md-10 col-sm-10 col-xs-10 zeroPadding">
			<div id="machinetooltypeconfig" class="table-responsive">
				<table class="table" id="machinetooltypeconfigTable">
					<tr>
						<th>Model Number</th>
						<th>Maker</th>
					</tr>
					...
					<c:forEach var="machinetoolType" items="${machinetoolTypes}">
						<tr>
							<td title="${machinetoolType.modelNumber}">${machinetoolType.modelNumber}</td>
							<td title="${machinetoolType.maker}">${machinetoolType.maker}</td>
						</tr>
					</c:forEach>
				</table>
			</div>
...
$(document).ready(function() {
	...
	$('#NewRow').click(function () {
		...
		hideErrorMsg();
		showSaveCancel();
		var tbodyAppend = '<tr id="machinetoolTypeConfigNewRow"><td><input name="modelNumber" maxlength="64"></td>'
		+'<td><input name="maker" maxlength="64"></td>';
		$('#machinetooltypeconfigTable tbody').append(tbodyAppend);
			});
	$('#Save').click(function () {
		...
		var machinetoolTypeData = {};
		machinetoolTypeData["modelNumber"] = modelNumber;
		machinetoolTypeData["maker"] = maker;
		var machinetoolTypeDataJson = JSON
			.stringify(machinetoolTypeData);
		var platformresturl = '${platformUrl}';
		platformresturl += "/machine_tools/types";
		var token = '${user_auth}';
		$.ajax({
			type : "POST",
			contentType : 'application/json; charset=utf-8',
			url : platformresturl,
			data : machinetoolTypeDataJson,
			dataType : 'text',
			headers:{
				'Authorization':token,
			...
			success : function (responsedata,
				...
				+'<td title ="'+maker+'">'+maker+'</td>';
				$("#machinetoolTypeConfigNewRow").remove();
				$('#machinetooltypeconfigTable tbody').append(tbodyAppend);
