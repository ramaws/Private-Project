...
<body>
  ...
  <div id="sidebar-wrapper" class="sidebar-toggle">
			<ul class="sidebar-nav">
		    	...
		    	<li>
		      		<a href='<c:url value="/config/workpiecetypes"/>'>WorkPiece Type</a>
		    	</li>
		    	<li  style="display: none;">
		      		<a href='<c:url value="/config/machinetooltypes"/>'>Machinetool Type</a>
		    	</li>
		    	<li  style="display: none;">
		      		<a href='<c:url value="/config/machine_tools"/>'>Machinetool-MDU</a>
		    	</li>
		  	</ul>
		</div>
  </div>
	<div class="col-lg-10 col-md-10 col-sm-10 col-xs-10 zeroPadding">
			<div id="tooltypeconfig" class="table-responsive">
						<table class="table" id="tooltypeconfigTable">
							<tr>
								<th>Model Number</th>
								<th>Diameter</th>
								<th>Maker</th>
							...
							<c:forEach var="tooltype" items="${toolTypes}">
								<tr>
									<td title="${tooltype.modelNumber}">${tooltype.modelNumber}</td>
									<td title="${tooltype.diameter}">${tooltype.diameter}</td>
									<td title="${tooltype.maker}">${tooltype.maker}</td>
								</tr>
							</c:forEach>
						</table>
		</div>
...
$(document).ready(function() {
	...
	$('#NewRow').click(function () {
		...
		hideErrorMsg();
		showSaveCancel();
		var tbodyAppend = '<tr id="toolTypeConfigNewRow"><td><input name="modelNumber" maxlength="64"></td>'
		+'<td><input name="diameter" maxlength="5"></td><td><input name="maker" maxlength="64"></td>';
		$('#tooltypeconfigTable tbody').append(tbodyAppend);
			});
	$('#Save').click(function () {
		...
		if(isBlank(diameter) ){
			showErrorMsg();
			showErrorMessage("Diameter is Mandatory please enter a value");
			hideErrorMessageAfter3Sec();
			$("input[name=diameter").focus();
			return;
		}
		if(!validateDecimal(diameter)){
			showErrorMsg();
			showErrorMessage("Diameter is a Real Number please enter a correct value");
			hideErrorMessageAfter3Sec();
			return;
		}
		...
		var toolTypeData = {};
		toolTypeData["modelNumber"] = modelNumber;
		toolTypeData["diameter"] = diameter;
		toolTypeData["maker"] = maker;
		var toolTypeDataJson = JSON
			.stringify(toolTypeData);
		var platformresturl = '${platformUrl}';
		platformresturl += "/tool_types";
		var token = '${user_auth}';
		$.ajax({
			type : "POST",
			contentType : 'application/json; charset=utf-8',
			url : platformresturl,
			data : toolTypeDataJson,
			dataType : 'text',
			headers:{
				'Authorization':token,
			...
			success : function (responsedata,
				...
				+'<td title= "'+diameter+'">'+diameter+'</td><td title= "'+maker+'">'+maker+'</td>';
				$("#toolTypeConfigNewRow").remove();
				$('#tooltypeconfigTable tbody').append(tbodyAppend);
